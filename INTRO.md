# ClawRyderz — an agent that turns a Telegram channel into a knowledge website
...
- 🔗 **Live site:** https://clawryderz.spirittree.dev — *gated; you'll be sent a username + password separately.*
- A floating 🐯 **"Ask Bridg3"** chat panel on every page: ask a plain-English
...
```
Telegram channel
  → Bridg3bot (read-only poller; Telethon for one-time historical backfill)
  ...
  → Ask Bridg3 Q&A (Opus, vault-grounded) via a local FastAPI server + cloudflared tunnel
```
...
Clone the **public template** and self-host it for *any* channel/topic (mycology,
